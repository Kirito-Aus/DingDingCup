...
def get_Q12_app_files():
    ...
    # 2）读入用户后15天使用APP情况统计表
    print('2）开始读入用户后15天使用App情况统计表')
    last_15day_df = pd.read_csv('./Results/last_15day_app_daily_use_time.csv')
    # last_15day_df = last_15day_df.set_index('uid')
    print('开始为每个类型APP创建一张表')
    # 开始为每个类型的App创建一张表
    for num in range(1, 21):
        print(f'开始创建APP = {num}')
        # 分别为每个类型产生一个表
        each_day_df = first_15day_df.copy()

        last_15day_tmp = last_15day_df[last_15day_df['class'] == num][['uid', 'duration']]

        each_day_df = each_day_df.merge(last_15day_tmp, how='left', on='uid').fillna(0)
        each_day_df = each_day_df.rename(columns={'duration':'daily_duration'})

        print(f'开始写出{num}')
        each_day_df.to_csv(f'./Results/Q12EachClass/class_{num}.csv', index=False)
